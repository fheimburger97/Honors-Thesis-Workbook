...
income_ethnic_section_count <- intersection_join %>% 
  ...
  #rename("Median Income" = "median_income") %>%
  rename("Latinx Ethnic Food Section?" = "Is.there.a.Hispanic.Latinx.ethnic.foods.section.") %>%
  rename("Percent with Latinx Ethnic Sections" = "perc_with_LatinX_ethnic_section") %>%
  st_drop_geometry()
...
income_ethnic_section_count_present <- read.csv("latinx ethnic section (reorder for presentation).csv") %>%
  ...
  rename("Latinx Ethnic Food Section?" = "Latinx.Ethnic.Food.Section.") %>%
  rename("% with Latinx Ethnic Sections" = "Percent.with.Latinx.Ethnic.Sections")
...
income_social_count <- intersection_join %>% 
  ...
  #rename("Median Income" = "median_income") %>%
  rename("Social Sustainability Signage?" = "Is.there.signage.or.visible.documentation.of.social.justice.work.or.actions.") %>%
  rename("Percent with Social Signage" = "perc_with_social_signage") %>%
  st_drop_geometry()
...
ethnicity_social_count <- intersection_2 %>%
  ...
  rename("Majority Ethnicity" = "majority_ethnicity") %>%
  rename("Social Sustainability Signage?" = "Is.there.signage.or.visible.documentation.of.social.justice.work.or.actions.") %>%
  rename("% with Social Signage" = "perc_with_social_signage") %>%
  st_drop_geometry()
income_social_count
...
income_social_count_present <- read.csv("soc_sustain_for_present.csv") %>%
  ...
  rename("Number of Retailers" = "Number.of.Retailers") %>%
  rename("% with Social Signage" = "Percent.with.Social.Signage")
...
```{r}
primary_table %>% 
  filter(str_detect(`If.so..what.kind.of.social.justice.work.`, "COVID")) %>% select(Name.of.Food.Retailer, If.so..what.kind.of.social.justice.work.)
