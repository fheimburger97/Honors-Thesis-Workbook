...
plot(just_geo_2, graticule = st_crs(4326), key.pos = 1)
...
#map
#income_map_with_retail
#class(ethnicity_with_retail)
just_geo_2 <- mission_percents_clean %>%
  select("geometry") %>%
  st_as_sf(crs = "WGS84")
just_geo_2 <- st_set_crs(just_geo_2, "WGS84")
intersection <- st_intersection(x = just_geo_2, y = just_mission_stores)
mission_percents_clean
retail_map
```
```{r}
