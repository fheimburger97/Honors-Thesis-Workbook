...
##```{r}
intersection <- st_intersection(x = incomemap_with_food_retailers$median_income, y = dots)
class(retail_map)
# Plot intersection
...
price_table <- primary_table %>%
  ...
  st_drop_geometry()
price_table$mean_quartile <- rowMeans(price_table_2[ , c(1,2)], na.rm=TRUE)
price_table_2

price_table_2 <- price_table %>%
  select(egg_quartile, milk_quartile, orange_quartile, avocado_quartile, carrots_quartile, brocolli_quartile) %>%
  st_drop_geometry() 
  #mutate(mean_quartile = apply(price_table[1:2], MARGIN=1, FUN=mean, na.rm=TRUE))
price_table_2$mean_quartile <- rowMeans(price_table_2[ , c("egg_quartile", "milk_quartile", "orange_quartile", "avocado_quartile", "carrots_quartile", "brocolli_quartile")], na.rm=TRUE)
price_table_2

class(price_table_2)
testing <- price_table_2 %>% st_drop_geometry()
testing

price_table_2$new <- rowMeans(price_table_2[ , c(1,2)], na.rm=TRUE)
price_table_2

price_numberic <- as.numeric(price_table_2)
price_table_2$new <- rowMeans(price_table_2[ , c("egg_quartile","milk_quartile")], na.rm=TRUE)

price_table_2$mean_quartile <- price_table_2[, apply(.SD, mean), na.rm=TRUE]
price_table_2

#price_table$mean_quartile <- rowMeans(price_table[,c('egg_quartile', 'bread_quartile')], na.rm=TRUE)
```

```{r}
dots <- tm_shape(price_table) + tm_dots(size=.1, col=c("egg_quartile", "milk_quartile"), palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("median_income") +
             tm_layout(main.title="Median Household Income & Food Retailers",
