...
```{r}
data_sf <- incomemap_with_food_retailers %>% st_as_sf()
...
# View result solution<-as.data.frame(table(unlist(record)))
solution <- as.data.frame(table(unlist(intersection_join$geom))) # using table
solution
```
...
```{r}
#EBT Test
intersection_EBT <- st_intersection(x = just_geo, y = just_mission_stores)
intersection_join_EBT <- st_join(intersection, incomejoin_test, join = st_intersects)
intersection_join_EBT
