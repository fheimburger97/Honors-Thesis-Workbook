...
Google API Key
#```{r}
register_google(key = "AIzaSyCf_Byte2gNDXotoxPbQ3Nbd77ZpJalxDs", write = TRUE)
#```
...
```{r}
retail_data <- read_csv("Copy of Food Retailer Survey (Edits 3_29) - Form Responses 1.csv")
retail_data

```

```{r}
retail_data_with_coord <- mutate_geocode(retail_data, location = Address, output = "latlona")
retail_data_with_coord
```

```{r}
write.csv(retail_data_with_coord,"C:\\Users\\fheim\\Desktop\\Test\\Retail_Data_w_Coords.csv", row.names = TRUE)
```
