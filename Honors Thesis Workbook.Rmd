---
title: "Urban Food Sovereignty: A Case Study of San Francisco's Mission District"
authors: Fischer Heimburger (@fheimburger97)
output:
  html_document:
    df_print: paged
---
...
incomejoin_test <-left_join(gfile, medianincome_test, by = "GEOID") %>%
  rename("Median Household Income" = "median_income")
incomejoin_test
```
...
incomemap_2_leaf <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha=.75) +
...
incomemap_2 <- tmap_leaflet(incomemap_2_leaf) %>% addTiles()
incomemap_2
```
...
```{r}
mission_percents_clean_2 <- mission_percents_clean %>%
  rename("Majority Ethnicity" = "Majority")
ethnic_map_2 <- tm_basemap("OpenStreetMap") + tm_shape(mission_percents_clean_2) +
  tm_polygons("Majority Ethnicity", alpha = .75) +
  tm_style("watercolor") +
...
-
```{r}
ethnic_map <- tm_basemap("OpenStreetMap") + tm_shape(mission_percents_clean) +
  tm_polygons("Majority", alpha = .75) +
  tm_style("watercolor") +
  tm_layout(main.title = "Racial Typography",
            main.title.position = "centre",
            main.title.size = 1.6) +
...
```{r warning=FALSE}
dots <- tm_basemap("OpenStreetMap") + tm_shape(retail_map) + tm_dots(size = .05, col="black") + tm_add_legend(shape = "tm_dots", labels = ('Food Retailers'), col = "black")
...
*
```{r warning=FALSE}
mission_percents_clean_title <- mission_percents_clean %>%
  rename("Majority Ethnicity" = "Majority")
dots <- tm_basemap("OpenStreetMap") + tm_shape(retail_map) + tm_dots(size = .05, col="black") + tm_add_legend(shape = "tm_dots", labels = ('Food Retailers'), col = "black")
ethnicity_map_with_food_retailers <-
  tm_shape(mission_percents_clean_title) +
  tm_polygons("Majority Ethnicity", alpha = .75) +
  tm_style("watercolor") +
...
```{r warning=FALSE}
dots_ebt <- tm_basemap("OpenStreetMap") + tm_shape(retail_map) + tm_dots(size=.05, col="EBT Accepted?", palette=c(No='red', Yes='black'))
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Food Retailers",
...
#### Maps
```{r}
incomejoin_test_title <- incomejoin_test
  
primary_table_price_title <- primary_table_price %>%
  rename("Number of Produce Options" = "How.many.types.of.fresh.produce.are.available.") %>%
  rename("Price Quartile" = "price_quartile")
  
dots_price <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_price_title) + tm_dots(size=.05, col="Price Quartile", palette="Set1", showNA = TRUE)
incomemap_with_food_retailers <- tm_shape(incomejoin_test_title) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Price Quartiles",
                       main.title.position = "centre",
                       main.title.size = 1.6) +
             tm_legend(position = c("right", "top"),
             legend.outside = TRUE,
...
ethnicity_plus_price <- ethnicity_map_with_food_retailers + dots_price + 
  tm_layout(main.title = "Majority Ethnicity & Price Quartiles",
            main.title.position = "centre",
            main.title.size = 1.6)

tmap_arrange(income_plus_price, ethnicity_plus_price)
income_plus_price
...
income_price_count <- primary_table_price %>% 
  rename("median_income" = "Median Household Income.x") %>%
...
price_table_percent_income <- read_csv("Income Price Table (percent).csv") %>%
  rename("Median Household Income" = "median_income") %>%
  rename("Price Quartile" = "price_quartile") %>%
  rename("% of Respective Quartile" = "percent of quartile") %>%
  rename("Number of Retailers" = "number of retailers") %>%
  print(n = 13)
price_table_percent_ethnicity <- read_csv("Ethnicity Price Table (percent).csv") %>%
  rename("Majority Ethnicity" = "majority_ethnicity") %>%
  rename("Price Quartile" = "price_quartile") %>%
  rename("% of Respective Quartile" = "percent of quartile") %>%
  rename("Number of Retailers" = "number of retailers")
...
selection_for_presentation_price_income <- price_table_percent_income %>% filter(`Price Quartile` == 1| `Price Quartile` == 4)
selection_for_presentation_price_ethnicity <- price_table_percent_ethnicity %>% filter(`Price Quartile` == 1| `Price Quartile` == 4)
selection_for_presentation_price_income
selection_for_presentation_price_ethnicity

#selection_for_presentation_price_income %>% kable(caption = "Income & Price Quartile (filtered for quartiles 1 & 4)")
#selection_for_presentation_price_ethnicity %>% kable(caption = "Ethnicity & Price Quartile (filtered for quartiles 1 & 4)")
```
...
tmap_arrange(income_plus_ebt, ethnicity_plus_ebt)
```
#### Tables
```{r}
ebt_count <- primary_table %>%
  ...
  mutate(Percent = (n / sum(n)) * 100) %>%
  rename("Number of Retailers" = "n") %>%
  rename("EBT Accepted?" = "EBT.Accepted.")
ebt_count

income_ebt_count <- intersection_join %>% 
  group_by(`Median Household Income`, EBT.Accepted.) %>% 
  count() %>%
  filter(EBT.Accepted. == "Yes") %>%
  mutate(`% of EBT Accepted` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  rename("EBT Accepted?" = "EBT.Accepted.") %>%
  #slice(match(costum_order, median_income)) %>%
  st_drop_geometry()
income_ebt_count
...
#### Maps
```{r}
primary_table_prod_labels <- primary_table %>%
  rename("% of Store is Produce" = "What.percentage.of.the.store.is.produce.") %>%
  rename("Produce Available?" = "Is.there.fresh.produce.available.") %>%
  rename("Signage other than English?" = "Is.there.signage.in.other.languages.than.English.") %>%
  rename("Signage in Spanish?" = "Is.there.signage.in.Spanish.")

dots_health <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_prod_labels) + tm_dots(size=.05, col="% of Store is Produce", palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Food Retailers",
...
tmap_arrange(income_plus_health, ethnicity_plus_health)
```
#### Tables
```{r}
income_health_count <- intersection_join %>% 
  group_by(`Median Household Income`, What.percentage.of.the.store.is.produce.) %>% 
  count() %>%
  mutate(percent = (n / nrow(primary_table)) * 100) %>%
  rename("number of retailers" = "n")
...
#### Maps
```{r}
dots_produce <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_prod_labels) + tm_dots(size=.05, col="Produce Available?", palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Food Retailers",
...
tmap_arrange(income_plus_produce, ethnicity_plus_produce)
```
#### Tables
```{r}
carry_produce_counts <- primary_table_price %>%
  ...
  mutate(Percent = (n / sum(n)) * 100) %>%
  rename("Number of Retailers" = "n") %>%
  rename("Fresh Produce Available?" = "Is.there.fresh.produce.available.")
...
income_produce_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.fresh.produce.available.) %>% 
  count() %>%
  filter(Is.there.fresh.produce.available. == "Yes") %>%
  mutate(`% has Produce` = (n / sum(n)) * 100) %>%
  #mutate(percent_produce_in_tract = (n / work_table$n * 100)) %>%
  rename("Number of Retailers" = "n") %>%
  rename("Fresh Produce?" = "Is.there.fresh.produce.available.") %>%
  #slice(match(costum_order, median_income)) %>%
  st_drop_geometry()
...
#### Maps
```{r}
primary_table_title <- primary_table %>%
  rename("Number of Produce Options" = "How.many.types.of.fresh.produce.are.available.") %>%
  #rename("Median Household Income" = "median_income") %>%
  rename("Latinx Ethnic Food Section?" = "Is.there.a.Hispanic.Latinx.ethnic.foods.section.") %>%
  rename("Social Sustainability Signage?" = "Is.there.signage.or.visible.documentation.of.social.justice.work.or.actions.") %>%
  rename("Environmental Sustainability Signage?" = "Is.there.signage.or.visible.documentation.of.environmental.work.or.actions.") %>%
  rename("Organic Produce Available?" = "Is.there.organic.produce.available.") %>%
  rename("Local Produce Available?" = "Is.there.local.produce.available.")
  

dots_quantity <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_title) + tm_dots(size=.05, col="Number of Produce Options", palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test_title) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Produce Diversity",
                       main.title.position = "centre",
                       main.title.size = 1.3) +
             tm_legend(position = c("right", "top"),
             legend.outside = TRUE,
...
ethnicity_plus_produce_quantity <- ethnicity_map_with_food_retailers + dots_quantity +
  tm_layout(main.title = "Majority Ethnicity and Produce Diversity",
            main.title.position = "centre",
            main.title.size = 1.6)
income_plus_produce_quantity
ethnicity_plus_produce_quantity
tmap_arrange(income_plus_produce_quantity, ethnicity_plus_produce_quantity)
```
#### Tables
```{r}
...
income_produce_quantity_count <- produce_quantity_w_bin %>% 
  group_by(`Median Household Income`, produce_quantity_bins) %>% 
  count() %>%
  mutate(Percent = (n / nrow(primary_table)) * 100) %>%
  rename("Number of Retailers" = "n")
...
ethnicity_produce_quantity_count <- produce_quantity_w_bin_2 %>%
  rename("Majority Ethnicity" = "Majority") %>%
  group_by(`Majority Ethnicity`, produce_quantity_bins) %>%
  count() %>%
  mutate(Percent = (n / nrow(primary_table)) * 100) %>%
  rename("Number of Retailers" = "n")
...
per_produce_quantity_income <- read_csv("produce_quantity_income (percent).csv") %>%
  rename("Median Household Income" = "median_income") %>%
  rename("# of Produce Options" = "produce_quantity_bins") %>%
  rename("% of Produce Quantities" = "percent_quantity") %>%
  rename("# of Retailers" = "number of retailers") %>% 
  filter(`# of Produce Options` == "0 to 10"| `# of Produce Options` == "41 to 50+")
...
per_produce_ethnicity_for_presentation <- per_produce_quantity_ethnicity %>% filter(`# of Produce Options` == "0 to 10"| `# of Produce Options` == "41 to 50+")
per_produce_ethnicity_for_presentation
```
...
#### Maps
```{r}
dots_language <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_prod_labels) + tm_dots(size=.05, col="Signage other than English?" , palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Food Retailers",
...
tmap_arrange(income_plus_language, ethnicity_plus_language)
```
#### Tables
```{r}
...
income_language_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.signage.in.other.languages.than.English.) %>% 
  count() %>%
  filter(Is.there.signage.in.other.languages.than.English. == "Yes") %>%
  mutate(`% with other Language` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  rename("Non-English Signage?" = "Is.there.signage.in.other.languages.than.English.") %>%
  #slice(match(costum_order, median_income)) %>%
  st_drop_geometry()
...
ethnicity_language_count <- intersection_2 %>%
  rename("Majority Ethnicity" = "Majority") %>%
  group_by(`Majority Ethnicity`, Is.there.signage.in.other.languages.than.English.) %>%
  count() %>%
  filter(Is.there.signage.in.other.languages.than.English. == "Yes") %>%
  mutate(`% with other Language` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
...
#### Maps
```{r}
dots_other_languages <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_prod_labels) + tm_dots(size=.05, col="Signage in Spanish?" , palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Food Retailers",
...
tmap_arrange(income_plus_languages, ethnicity_plus_languages)
```
#### Tables
```{r}
other_language_counts <- primary_table %>%
  ...
  mutate(Percent = (n / sum(n)) * 100) %>%
  rename("Number of Retailers" = "n") %>%
  rename("Spanish Signage?" = "Is.there.signage.in.Spanish.")
other_language_counts

income_other_language_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.signage.in.Spanish.) %>% 
  count() %>%
  filter(Is.there.signage.in.Spanish. == "Yes") %>%
  mutate(`% with Spanish Signage` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  rename("Spanish Signage?" = "Is.there.signage.in.Spanish.") %>%
  #slice(match(costum_order, median_income)) %>%
  st_drop_geometry()
...
ethnicity_other_language_count <- intersection_2 %>%
  rename("Majority Ethnicity" = "Majority") %>%
  group_by(`Majority Ethnicity`, Is.there.signage.in.Spanish.) %>%
  count() %>%
  filter(Is.there.signage.in.Spanish. == "Yes") %>%
  mutate(`% with Spanish Signage` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
...
#### Maps
```{r}
dots_ethnic_section <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_title) + tm_dots(size=.05, col="Latinx Ethnic Food Section?" , palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test_title) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Latinx Ethnic Food Sections",
                       main.title.position = "centre",
                       main.title.size = 1.2) +
             tm_legend(position = c("right", "top"),
             legend.outside = TRUE,
...
ethnicity_plus_ethnic <- ethnicity_map_with_food_retailers + dots_ethnic_section +
  tm_layout(main.title = "Ethnicity and Latinx Ethnic Food Sections",
            main.title.position = "centre",
            main.title.size = 1.6)
income_plus_ethnic
ethnicity_plus_ethnic
tmap_arrange(income_plus_ethnic, ethnicity_plus_ethnic)
```
#### Tables
```{r}
...
income_ethnic_section_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.a.Hispanic.Latinx.ethnic.foods.section.) %>% 
  count() %>%
  filter(Is.there.a.Hispanic.Latinx.ethnic.foods.section. == "Yes") %>%
  mutate(perc_with_LatinX_ethnic_section = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  #rename("Median Income" = "median_income") %>%
  rename("Latinx Food Section?" = "Is.there.a.Hispanic.Latinx.ethnic.foods.section.") %>%
  rename("% w/ Latinx Ethnic Sections" = "perc_with_LatinX_ethnic_section") %>%
  st_drop_geometry()
...
income_ethnic_section_count_present <- read.csv("latinx ethnic section (reorder for presentation).csv") %>%
  rename("# of Retailers" = "Number.of.Retailers") %>%
  rename("Median Household Income" = "Median.Income") %>%
  rename("Latinx Food Section?" = "Latinx.Ethnic.Food.Section.") %>%
  rename("% w/ Latinx Ethnic Sections" = "Percent.with.Latinx.Ethnic.Sections")
ethnic_section_count
income_ethnic_section_count
ethnicity_ethnic_section_count
income_ethnic_section_count_present
```
...
ethnic_section_count_non_nat <- primary_table %>%
  ...
  mutate(percent = (n / sum(n)) * 100) %>%
  rename("number of retailers" = "n") %>%
  rename("Options Besides National Name Brand Ethnic Foods?" = "Are.there.options.besides.national.name.brand.ethnic.foods.") %>%
  rename("Latinx Ethnic Food Section?" = "Is.there.a.Hispanic.Latinx.ethnic.foods.section.") %>%
  rename("Percent" = "percent")

income_ethnic_section_count_non_nat <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.a.Hispanic.Latinx.ethnic.foods.section., Are.there.options.besides.national.name.brand.ethnic.foods.) %>% 
  count() %>%
  filter(Is.there.a.Hispanic.Latinx.ethnic.foods.section. == "Yes") %>%
  ...
  rename("Latinx Ethnic Food Section?" = "Is.there.a.Hispanic.Latinx.ethnic.foods.section.") %>%
  rename("Options Besides National Name Brand Ethnic Foods?" = "Are.there.options.besides.national.name.brand.ethnic.foods.") %>%
  rename("Percent with Latinx Non-national Ethnic Sections" = "perc_with_LatinX_non_national_ethnic_sections") %>%
...
#### Maps
```{r}
dots_environment <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_title) + tm_dots(size=.05, col="Environmental Sustainability Signage?" , palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
             tm_layout(main.title="Median Household Income & Food Retailers",
...
income_plus_env
ethnicity_plus_env
tmap_arrange(income_plus_env, ethnicity_plus_env)
```
####Tables
```{r}
env_signage_count <- primary_table %>%
  ...
  mutate(Percent = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  rename("Env. Sustainability Signage?" = "Is.there.signage.or.visible.documentation.of.environmental.work.or.actions.")
env_signage_count

income_env_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.signage.or.visible.documentation.of.environmental.work.or.actions.) %>% 
  count() %>%
  filter(Is.there.signage.or.visible.documentation.of.environmental.work.or.actions. == "Yes") %>%
  mutate(`% w/ Env. Signage` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  rename("Env. Sustainability Signage?" = "Is.there.signage.or.visible.documentation.of.environmental.work.or.actions.") %>%
  #slice(match(costum_order, median_income)) %>%
  st_drop_geometry()
...
ethnicity_env_count <- intersection_2 %>%
  rename("Majority Ethnicity" = "Majority") %>%
  group_by(`Majority Ethnicity`, Is.there.signage.or.visible.documentation.of.environmental.work.or.actions.) %>%
  count() %>%
  filter(Is.there.signage.or.visible.documentation.of.environmental.work.or.actions. == "Yes") %>%
  mutate(`% w/ Env. Signage` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
...
#### Maps
```{r}
dots_social <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_title) + tm_dots(size=.05, col="Social Sustainability Signage?" , palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test_title) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
...
tmap_arrange(income_plus_social, ethnicity_plus_social)
```
#### Tables
```{r}
...
income_social_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.signage.or.visible.documentation.of.social.justice.work.or.actions.) %>% 
  count() %>%
  ...
  #rename("Median Income" = "median_income") %>%
  rename("Social Sustainability Signage?" = "Is.there.signage.or.visible.documentation.of.social.justice.work.or.actions.") %>%
  rename("% w/ Social Signage" = "perc_with_social_signage") %>%
  st_drop_geometry()
...
income_social_count_present <- read.csv("soc_sustain_for_present.csv") %>%
  rename("Median Income" = "Median.Income") %>%
  rename("Social Sustainability Signage?" = "Social.Sustainability.Signage.") %>%
  rename("# of Retailers" = "Number.of.Retailers") %>%
  rename("% w/ Social Signage" = "Percent.with.Social.Signage")
income_social_count_present
```

###Organic
####Maps
```{r}
dots_organic <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_title) + tm_dots(size=.05, col="Organic Produce Available?" , palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
...
tmap_arrange(income_plus_organic, ethnicity_plus_organic)
```
...
income_organic_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.organic.produce.available.) %>% 
  count() %>%
  filter(Is.there.organic.produce.available. == "Yes") %>%
  mutate(`% w/ Organic Produce` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  rename("Organic Produce Available?" = "Is.there.organic.produce.available.") %>%
...
ethnicity_organic_count <- intersection_2 %>%
  rename("Majority Ethnicity" = "Majority") %>%
  group_by(`Majority Ethnicity`, Is.there.organic.produce.available.) %>%
  count() %>% 
  filter(Is.there.organic.produce.available. == "Yes") %>%
  mutate(`% w/ Organic Produce` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
...
####Maps
```{r}
dots_local <- tm_basemap("OpenStreetMap") + tm_shape(primary_table_title) + tm_dots(size=.05, col="Local Produce Available?" , palette="Set1")
incomemap_with_food_retailers <- tm_shape(incomejoin_test) +
             tm_style("watercolor") +
             tm_polygons("Median Household Income", alpha = .75) +
...
tmap_arrange(income_plus_local, ethnicity_plus_local)
```
...
income_local_count <- intersection_join %>% 
  group_by(`Median Household Income`, Is.there.local.produce.available.) %>% 
  count() %>%
  filter(Is.there.local.produce.available. == "Yes") %>%
  mutate(`% w/ Local Produce` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
  rename("Local Produce Available?" = "Is.there.local.produce.available.") %>%
  #slice(match(costum_order_3, median_income)) %>%
  st_drop_geometry()
...
ethnicity_local_count <- intersection_2 %>%
  rename("Majority Ethnicity" = "Majority") %>%
  group_by(`Majority Ethnicity`, Is.there.local.produce.available.) %>%
  count() %>% 
  filter(Is.there.local.produce.available. == "Yes") %>%
  mutate(`% w/ Local Produce` = (n / sum(n)) * 100) %>%
  rename("# of Retailers" = "n") %>%
